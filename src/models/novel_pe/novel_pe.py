...
class NOVEL_PE(LightningModule):
    ...
    def encode_batch(self, batch):
        self.nerf.encode(images=batch["src_rgbs"],
                         pos_encodings=batch["src_pos_encodings"],
                         depths=batch["src_depths"],
                         depths_std=batch["src_depth_stds"],
                         extrinsics=batch["src_extrinsics"],
                         intrinsics=batch["src_intrinsics"])
        
        self.nerf.encode_gen(gen_pos_encoding=batch["gen_pos_encoding"],
                             extrinsics=batch["gen_extrinsics"],
                             intrinsics=batch["gen_intrinsics"],
                             image_shape=batch["target_rgb"].shape[-2:])
        
        self.nerf.encode_tgt(tgt_pos_encoding=batch["target_pos_encoding"],
