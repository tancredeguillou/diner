...
def write_prediction(model, dataloaders, args):
    ...
    for loader in dataloaders:
        for batch_idx, sample in tqdm.tqdm(enumerate(loader), total=len(loader)):
            ...
            out_depth_path = os.path.join(args.outpath, src_path_stem + "_gt_pred_conf.png")
            #gt_depth_path = os.path.join(args.outpath, sample["dpath"][0])
            gt_depth_path = os.path.join(args.outpath, src_path_stem + "_TransMVSNet.png")
            ...
            if os.path.isfile(gt_depth_path):
                ...
                dst = Image.new('I', (gt_img.width + conf_img.width, conf_img.height))
                dst.paste(gt_img, (0, 0))
                dst.paste(conf_img, (gt_img.width, 0))
